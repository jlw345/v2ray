...
install_bbr() {
	...
	if [[ $is_bbr ]]; then
		...
		bash <(curl -s -L https://raw.githubusercontent.com/jlw345/bbr/master/bbr.sh)
	fi
}
install_lotserver() {
...
update_v2ray.sh() {
	if [[ $_test ]]; then
		...
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/jlw345/v2ray/master/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	fi
